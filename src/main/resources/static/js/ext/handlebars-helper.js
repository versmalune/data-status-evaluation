...
Handlebars.getContextPath = function () {
    var secondIndex = location.pathname.indexOf("/", 1);

    return location.pathname.substring(0, secondIndex); // "/abc"
}
...
Handlebars.getTemplate = function(name) {
    ...
    if (Handlebars.templates === undefined || Handlebars.templates[name] === undefined) {
        $.ajax({
            url : self.contextPath + '/js/templates/' + name + '.hbs',
            async : false,
            dataType: "text",
